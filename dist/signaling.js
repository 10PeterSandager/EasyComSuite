...
exports.applyBackupState = applyBackupState;
...
function connKey(c) { return `${c.from}-${c.to}-${c.channel}`; }
...
function applyBackupState(s, io) {
    connections.clear();
    groups.splice(0, groups.length);
    ...
    for (const [id, v] of clients.entries()) {
        if (!v.socketId)
            clients.delete(id); // ghost from old state — remove
    }
    s.connections.forEach(c => connections.set(`${c.from}-${c.to}-${c.channel}`, c));
    s.groups.forEach(g => groups.push(g));
    s.clients.forEach(({ id, data }) => {
        if (!clients.has(id))
            clients.set(id, { socketId: "", data });
        else
            clients.set(id, { ...clients.get(id), data });
    });
    audioRoutes.clear();
    s.audioRoutes.forEach(r => audioRoutes.set(`${r.deviceId}-${r.channel}`, r));
    bridgeChannelInfo = s.bridgeChannelInfo ?? [];
    ...
    const clientsObj = {};
    for (const [id, v] of clients.entries())
        clientsObj[id] = { ...v.data, status: v.socketId ? "online" : "offline" };
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("client:register", (client, cb) => {
            ...
            if (client.type === "mobile" || client.type === "remote" || client.type === "desktop") {
                ...
                (0, backup_1.emitBackupConfig)(socket);
                ...
                if (layout?.slots) {
                    ...
                    if (!hasConns) {
                        ...
                        layout.slots.filter(s => s && (s.id || s.clientId) && !s.isGroupLatch).forEach((slot, i) => {
                            ...
                            const outChannel = slot.outputChannel ?? 1;
            ...
            if (isRealDevice) {
                const hasHostRoute = Array.from(connections.values()).some(c => c.from === client.id && c.to === "producer-65");
                if (!hasHostRoute) {
        ...
        socket.on("connection:remove", ({ from, to, channel, bidirectional = true }) => {
            ...
            if (bidirectional)
                connections.delete(connKey({ from: to, to: from, channel }));
            broadcastRouting(io);
            save();
        });
        ...
        socket.on("talk:targeted", ({ targetClientId, slot, active }) => {
            const entry = [...clients.entries()].find(([id, c]) => c.socketId === socket.id && id !== "host-ui");
            if (!entry)
                return;
            const [clientId] = entry;
        ...
        socket.on("panel:layout:set", ({ clientId, slots, bgImage }) => {
            ...
            assignedSlots.forEach((slot, i) => {
                ...
                const outChannel = slot.outputChannel ?? 1; // which output channel the audio lands on
