...
type Role = "main" | "backup"

let _role: Role = "main"
let _backupUrl: string | null = null   // only relevant when role=main
let _syncInterval: ReturnType<typeof setInterval> | null = null
let _cachedState: PersistedState | null = null
let _lastSyncAt: number | null = null   // epoch ms (as backup: last time we received sync)
let _lastSendAt: number | null = null   // epoch ms (as main: last time we successfully sent)
let _syncOk = false                     // last sync attempt succeeded?

// callbacks injected from server.ts
let _getState: (() => PersistedState) | null = null
let _applyState: ((s: PersistedState, io: Server) => void) | null = null

// ─── Helpers ─────────────────────────────────────────────────────────────────

async function pushStateToBackup() {
  if (!_backupUrl || !_getState) return
  try {
    ...
    clearTimeout(t)
    _syncOk = res.ok
    if (res.ok) _lastSendAt = Date.now()
  } catch {
    _syncOk = false
  }
}
...
export function setupBackupRoutes(
  ...
  applyState: (s: PersistedState, io: Server) => void,
) {
  _getState   = getState
  ...
  app.post("/api/backup/config", (req, res) => {
    const { role, backupUrl } = req.body as { role?: Role; backupUrl?: string }

    if (backupUrl !== undefined) _backupUrl = backupUrl || null
  ...
  app.post("/api/backup/sync", (req, res) => {
    ...
    _cachedState = req.body as PersistedState
  ...
  app.post("/api/backup/push-backup-url", (_, res) => {
    if (_backupUrl) io.emit("backup:config", { backupUrl: _backupUrl })
    res.json({ ok: true, url: _backupUrl })
  })
}

// Called from signaling.ts after a client connects — tells them the backup URL
export function emitBackupConfig(socket: { emit: (ev: string, data: any) => void }) {
  if (_backupUrl) socket.emit("backup:config", { backupUrl: _backupUrl })
}
