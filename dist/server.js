...
const remoteDist = path_1.default.join(__dirname, "../../../easycom-remote/dist");
if (fs_1.default.existsSync(remoteDist)) {
    app.use("/remote", express_1.default.static(remoteDist));
    app.get("/remote/{*splat}", (_, res) => res.sendFile(path_1.default.join(remoteDist, "index.html")));
    console.log("📱 Remote app served at /remote");
}
// Serve easycom-desktop at /desktop
const desktopDist = path_1.default.join(__dirname, "../../../easycom-desktop/dist");
if (fs_1.default.existsSync(desktopDist)) {
    app.use("/desktop", express_1.default.static(desktopDist));
...
app.get("/cert", (_, res) => {
    ...
    res.sendFile(path_1.default.resolve(certPath));
});
...
app.get("/admin/clear-connections", (_, res) => {
    (0, signaling_1.clearAllConnections)(io);
    res.json({ ok: true });
});
app.post("/admin/factory-reset", (_, res) => {
    (0, signaling_1.factoryReset)(io);
    res.json({ ok: true });
});
app.get("/debug/state", (_, res) => {
    res.json((0, signaling_1.getDebugState)());
});
app.get("/debug", (_, res) => {
...
const server = (sslCert && sslKey)
    ? https_1.default.createServer({ cert: fs_1.default.readFileSync(sslCert), key: fs_1.default.readFileSync(sslKey) }, app)
...
const localHttpServer = (sslCert && sslKey) ? http_1.default.createServer(app) : null;
...
async function start() {
    try {
        ...
        /* MEDIASOUP */
        await (0, mediasoup_1.initMediasoup)();
        ...
        server.listen(PORT, "0.0.0.0", () => {
            ...
            tunnel_1.tunnelEvents.on("status", (state) => {
        ...
        signaling_1.streamDeckManager.setCallbacks((clientId, val) => { io.emit('client:talk', { clientId, isTalking: val }); }, (memberIds, val) => { io.emit('streamdeck:group:talk', { memberIds, val }); }, io);
        signaling_1.streamDeckManager.connect().catch(() => { });
